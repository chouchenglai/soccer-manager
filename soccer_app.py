...
# --- 主功能 ---
    ...
    with tab2:
        # 1. 自動偵測該報表的「初始本金」
        try:
            # 尋找類型為「初始」的那一行，獲取其金額作為基準線
            initial_cap = main_df[main_df['類型'] == '初始']['金額'].iloc[0]
        except:
            initial_cap = 60000  # 防呆機制：若無資料則預設為 60000
        ...
        def color_row(row):
            ...
            # --- A. 判斷單場勝負 (影響：類型、盈虧金額) ---
            if row['盈虧金額'] > 0:
                win_loss_style = 'color: green'
            elif row['盈虧金額'] < 0:
                win_loss_style = 'color: red'
            else:
                win_loss_style = 'color: black'

            # --- B. 判斷帳戶存亡 (影響：結算總分) ---
            # 只有當總分 >= 該表的初始本金，才會變綠，否則維持紅色警示
            if row['結算總分'] >= initial_cap:
                total_score_style = 'color: green'
            else:
                total_score_style = 'color: red'
