...
# ---------------------------------------------------------
# 3. 主頁面頂端：CCL-Soccer 旗艦版「中英先行」標題設計
# ---------------------------------------------------------
st.markdown("""
    <style>
        .ccl-brand-box {
            ...
            border-bottom: 2px solid #00c853; /* 加入獲利綠下底線 */
            margin-bottom: 25px;
        }
        .ccl-main-title {
        ...
        .ccl-prefix { color: #1E1E1E; }
        .ccl-suffix { color: #00c853; text-shadow: 2px 2px 4px rgba(0,200,83,0.15); }
        .ccl-chinese {
            font-size: 0.6em; 
            color: #555;
            letter-spacing: 5px;
    ...
    with tab3:
        ...
        if ready:
            if not main_df.empty:
                ...
                msg_box.info(f"🚀 分析中，共 {num_records} 單數據...")
