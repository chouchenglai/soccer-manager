   ...
   with tab5:
    ...
    if 'user_nickname' not in st.session_state:
        with st.form("simple_login"):
            name = st.text_input("請輸入您的暱稱：", placeholder="例如：玩家稱呼")
        ...
        with st.form("chat_form", clear_on_submit=True):
            # 支援回覆與編輯功能，從 session_state 抓取暫存內容
            initial_val = st.session_state.get('reply_to', "")
            msg = st.text_area("請輸入您要發表的內容", value=initial_val, height=100)
            
            if st.form_submit_button("送出留言"):
                if msg:
                    # 判斷標籤 (僅視覺區分)
                    tag = "管理員" if st.session_state.user_nickname in ["管理員", "阿來", "Admin"] else "訪客"
                    save_chat(st.session_state.user_nickname, msg, tag)
                    st.session_state.reply_to = "" # 送出後清空暫存
                    st.rerun()
        ...
        c_df = load_chat()
        if not c_df.empty:
            # 優先按「最後活動」排序，若無此欄位則按「時間」
            sort_key = "最後活動" if "最後活動" in c_df.columns else "時間"
            c_df = c_df.sort_values(by=sort_key, ascending=False)
            ...
            for idx, r in c_df.iterrows():
                ...
                # 佈局：左邊內容，右邊功能鈕
                col_text, col_btns = st.columns([75, 25])
                
                with col_text:
                    st.markdown(f"""
                        <div style="border-left: 5px solid {l_color}; padding-left: 15px; margin-bottom: 10px; background-color: #fcfcfc; padding: 10px; border-radius: 5px;">
                            <strong style="color: {l_color};">{r['暱稱']}</strong> 
                            <small style='color: #aaa; margin-left: 10px;'>{r['時間']}</small><br>
                            <div style="margin-top: 5px; color: #333;">{r['內容']}</div>
                        </div>
                    """, unsafe_allow_html=True)
                ...
                with col_btns:
                    ...
                    if b1.button("💬", key=f"rep_{idx}", help="回覆"):
                        st.session_state.reply_to = f"@{r['暱稱']} "
                        st.rerun()
                    if b2.button("✏️", key=f"edit_{idx}", help="編輯(帶入輸入框)"):
                        st.session_state.reply_to = r['內容']
                        st.rerun()
                    if b3.button("🗑️", key=f"del_{idx}", help="直接刪除"):
                        delete_chat(idx)
                        st.rerun()
        else:
            st.write("目前還沒有人留言，歡迎您加入討論！")
